...
async function renderReviews() {
  ...
  if (!cards.length) {
    ...
    if (wid) {
      grid.innerHTML = `<div class="rounded-3xl bg-cloud-card shadow-card overflow-hidden">
        <iframe src="${esc(wid.widgetUrl)}" title="Отзывы ${esc(wid.name)}" loading="lazy" class="w-full" style="height:440px;border:0" referrerpolicy="no-referrer-when-downgrade"></iframe>
      </div>`;
