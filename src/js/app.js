...
function renderSeriesTiles() {
  ...
  grid.innerHTML = series
    ...
    <button class="card text-left" data-series-tile="${esc(s.series)}">
      <div class="${(Array.isArray(rep.images) && rep.images.length) ? "bg-cloud" : "series-art"} h-40 rounded-2xl overflow-hidden grid place-items-center" style="--accent:${accent}">${pmedia(rep, 96, i < 3)}</div>
  ...
  const render = () => {
    ...
    box.innerHTML = hits
      ...
      <a class="flex w-full items-center gap-3 p-3 rounded-2xl hover:bg-black/[0.04] transition" href="${productLink(p)}">
        <span class="grid place-items-center w-11 h-11 rounded-xl bg-cloud shrink-0 overflow-hidden">${pmedia(p, 30)}</span>
