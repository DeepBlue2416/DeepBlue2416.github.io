...
function renderCategoryTiles() {
  ...
  grid.innerHTML = state.categories
    ...
    <button class="cat-tile" data-cat-tile="${esc(c.key)}" style="--accent:${c.accent || "#0582F6"}">
      <div class="${c.image ? "grid place-items-center h-28 sm:h-32 rounded-2xl media-plate overflow-hidden" : "cat-tile-art"}">${c.image ? `<img src="${esc(c.image)}" alt="${esc(c.title)}" class="max-h-[86%] max-w-[86%] object-contain" loading="lazy" decoding="async">` : deviceGlyph({ category: c.glyph || "accessory", colorHex: "#ffffff" }, 88)}</div>
