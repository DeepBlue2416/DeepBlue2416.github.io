...
function renderCategoryTiles() {
  ...
  grid.innerHTML = state.categories
    ...
    <button class="cat-tile" data-cat-tile="${esc(c.key)}" style="--accent:${c.accent || "#0582F6"}">
      <div class="${c.image ? "grid place-items-center aspect-square rounded-2xl media-plate overflow-hidden" : "cat-tile-art"}">${c.image ? `<img src="${esc(c.image)}" alt="${esc(c.title)}" class="cat-img h-full w-full object-contain p-4 sm:p-6" loading="lazy" decoding="async">` : deviceGlyph({ category: c.glyph || "accessory", colorHex: "#ffffff" }, 120)}</div>
...
async function openTradeInModal() {
  ...
  const html = `
    ...
    <p class="mt-3 text-xs text-ink-mute leading-relaxed">
      К выкупу принимаем устройства только в идеальном или хорошем состоянии и с ёмкостью
      аккумулятора от 80 %. Остальные — оцениваем индивидуально после диагностики в магазине.
    </p>
...
function renderReviews() {
  ...
  if (!grid || !sources.length) { section?.classList.add("hidden"); return; }

  grid.innerHTML = sources
    .map((s) => {
      // Официальный виджет площадки (если задан) — показываем iframe как есть.
      if (s.widgetUrl) {
        return `<div class="rounded-3xl bg-cloud-card shadow-card overflow-hidden">
          <iframe src="${esc(s.widgetUrl)}" title="Отзывы ${esc(s.name)}" loading="lazy"
            class="w-full" style="height:420px;border:0" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </div>`;
      }
      ...
      const featHtml = feats
        .map(
          (f) => `
        <div class="mt-4 pt-4 hairline first:mt-2 first:pt-0 first:border-t-0">
          <div class="flex items-center gap-2">
            <span class="grid place-items-center w-8 h-8 rounded-full bg-brand/10 text-brand text-sm font-semibold shrink-0">${esc((f.author || "•").slice(0, 1))}</span>
            <div class="min-w-0">
              <div class="font-medium text-sm leading-tight">${esc(f.author || "Покупатель")}</div>
              <div class="text-xs text-ink-mute">${esc(f.date || "")}</div>
            </div>
            <span class="ml-auto">${starRow(f.rating || 5, "w-3.5 h-3.5")}</span>
          </div>
          <p class="mt-2 text-sm text-ink-soft leading-relaxed">${esc(f.text || "")}</p>
        </div>`
        )
        .join("");

      return `<div class="rounded-3xl bg-cloud-card shadow-card p-6 sm:p-7 flex flex-col">
        <div class="flex items-center justify-between gap-3">
          <span class="font-semibold text-lg">${esc(s.name)}</span>
          <a href="${esc(s.url || "#")}" target="_blank" rel="noopener" class="text-xs text-ink-mute hover:text-brand transition">Открыть карточку</a>
        </div>
        <div class="mt-3 flex items-center gap-3">
          <span class="text-4xl font-semibold tracking-tight">${esc(s.rating || "5,0")}</span>
          <div>
            ${starRow(String(s.rating || "5").replace(",", "."), "w-4 h-4")}
            <div class="text-xs text-ink-mute mt-0.5">${esc(s.count || "")}</div>
          </div>
        </div>
        <div class="flex-1">${featHtml}</div>
        <a href="${esc(s.url || "#")}" target="_blank" rel="noopener" class="btn-soft w-full mt-5 justify-center">Смотреть все отзывы</a>
      </div>`;
    })
    .join("");
}
