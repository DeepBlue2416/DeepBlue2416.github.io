...
function openLeadModal(productId, presetText = "") {
  ...
  const sub = p
    ? `${[p.storage, p.color].filter((x) => x && x !== "—").join(" · ")} · ${fmt(p.priceCash)} ${state.currency}`
...
const FAQ_ITEMS = [
  ...
  { q: "Есть ли рассрочка?", a: "Да, доступна рассрочка 0-0-12 — без первоначального взноса и без переплат. Подробности уточняйте у оператора." },
