...
const state = {
  ...
  filters: { series: "", storage: "", watchSize: "", color: "", simSlots: [], simEsim: false },
...
function parseHash() {
  ...
  state.filters = { series: "", storage: "", watchSize: "", color: "", simSlots: [], simEsim: false };
}
...
function renderFilters() {
  ...
  wrap.innerHTML = defs.map((d) => select(d.key, d.label, d.values)).join("") + simBlock + sortSel;
  ...
  if (window.CONFIG && CONFIG.apiBase) {
    fetch(`${CONFIG.apiBase}/api/products`, { cache: "no-cache" })
      ...
      .then((fresh) => {
        ...
        if (JSON.stringify(fresh.products) !== JSON.stringify(state.all)) {
          applyCatalog(fresh);
          renderCatalog();
        }
...
const FAQ_ITEMS = [
  ...
  { q: "Как работает Trade-In?", a: "Вы сдаёте старое устройство в счёт нового. Ориентировочную стоимость можно рассчитать онлайн в калькуляторе Trade-In, а точную сумму назовём после бесплатной диагностики в магазине." },
  { q: "Можно купить в кредит?", a: "Да, оформляем кредит через банк-партнёр. Сумму, ставку, срок и итоговую переплату назовёт оператор при оформлении — условия зависят от решения банка." },
