...
function renderFilters() {
  ...
  const defs = [
    ...
    { key: "sim", label: "SIM", values: uniq(ctxPool, "sim") },
  ...
  const disp = (key, o) => (key === "sim" && window.simLabel ? window.simLabel(o) : o);
  const select = (key, label, values) => `<label class="relative">
      <select class="field appearance-none pr-9 text-sm py-2.5" data-filter="${key}">
        ...
        ${values.map((o) => `<option value="${esc(o)}" ${state.filters[key] === o ? "selected" : ""}>${esc(disp(key, o))}</option>`).join("")}
      </select>${chev}</label>`;
...
// дублей вроде «2 eSIM / 2 eSIM». Итоговые категории: «SIM + eSIM» / «SIM» / «eSIM» / «2 eSIM».
function normSim(s) {
  if (s == null) return "";
