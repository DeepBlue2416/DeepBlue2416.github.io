...
// Пустой набор фильтров. Раньше этот объект был выписан в четырёх местах
// (инициализация, разбор адреса, навигация, кнопка «Сбросить») — при добавлении
// нового фильтра одно из мест всегда забывалось, и фильтр «залипал» при
// переходе между разделами.
function blankFilters() {
  return { series: "", storage: "", watchSize: "", color: "", simSlots: [], simEsim: false, condition: "" };
}
...
const state = {
  ...
  // simSlots — набор выбранных чекбоксов «1 слот»/«2 слота» (может быть несколько
  // сразу, т.к. это независимые чекбоксы, а не один select); simEsim — чекбокс
  // «Есть поддержка eSIM». Оба фильтра ортогональны и сочетаются по И (AND).
...
// ------------------------------------------------------------------
// Кэш каталога в sessionStorage (мгновенный повторный рендер / возврат со страницы товара).
...
function readCatalogCache() {
  try {
    ...
    return d && Array.isArray(d.products) ? d : null;
  } catch (e) { return null; }
}
function writeCatalogCache(data) {
  try { sessionStorage.setItem(CATALOG_CACHE_KEY, JSON.stringify(data)); } catch (e) {}
}
...
// Применяет данные каталога к состоянию (индекс поиска, обложки, чистка избранного).
function applyCatalog(data) {
  // Скрытые позиции (hidden: true) убираем СРАЗУ на входе, а не в каждом месте
  // отдельно: иначе товар неизбежно где-нибудь протечёт — в поиске, в похожих,
  // в конфигураторе цвета или в счётчиках фильтров. Из products.js они при этом
  // не удаляются, цены у них продолжают обновляться, и вернуть товар в продажу
  // можно одним снятием флага в CRM.
  state.all = (data.products || []).filter((p) => !p.hidden);
  // Подпись СЫРЫХ данных — до нормализации SIM ниже (она меняет объекты на
  // месте). По ней фоновое обновление решает, изменилось ли что-то на самом
  // деле. Раньше сравнивались сырые данные с уже нормализованными — они не
  // совпадали никогда, и каталог перерисовывался при каждом открытии: картинки
  // мигали, а страница прыгала.
  state.rawSig = JSON.stringify(state.all);
  // Канонизируем тип SIM в самих данных — чтобы конфигуратор/SKU/заявка использовали
  // единые значения и не появлялись дубли «2 eSIM / 2 eSIM».
  for (const p of state.all) { if (p && "sim" in p) p.sim = normSim(p.sim) || "—"; }
  state.categories = data.categories || [];
  state.covers = (data.meta && data.meta.covers) || {};
  if (data.meta && data.meta.currency) state.currency = data.meta.currency;
  state.searchIndex = SmartSearch.build(state.all);
  buildIndexes(); // Map-индексы для O(1) выборок по категории/серии
  Store.pruneMissing(state.all.map((p) => p.id));
}

// Map-индексы: категория → товары, "категория||серия" → товары. Строим один раз
// при загрузке данных, чтобы не перебирать весь массив на каждый рендер.
function buildIndexes() {
  const byCat = new Map();
  const bySeries = new Map();
  for (const p of state.all) {
    // catOf: б/у и витринные уходят в отдельный раздел каталога, хотя в самом
    // товаре категория остаётся прежней (её ждут фиды и характеристики).
    const cat = window.catOf ? window.catOf(p) : p.category;
    if (!byCat.has(cat)) byCat.set(cat, []);
    byCat.get(cat).push(p);
    const sk = cat + "||" + p.series;
    if (!bySeries.has(sk)) bySeries.set(sk, []);
    bySeries.get(sk).push(p);
  }
  state.byCategory = byCat;
  state.bySeries = bySeries;
}

// Категории для первого уровня каталога: те, что заданы в CRM, плюс
// виртуальный раздел «Б/у и витрина» — но только если такие позиции есть.
function catalogCategories() {
  const list = (state.categories || []).filter((c) => (state.byCategory && state.byCategory.get(c.key) || []).length);
  const uc = window.USED_CATEGORY;
  if (uc && state.byCategory && (state.byCategory.get(uc.key) || []).length) list.push(uc);
  return list;
}
...
function categoryPool() {
  if (!state.category) return state.all;
  return (state.byCategory && state.byCategory.get(state.category)) || [];
}
...
function seriesOfCategory(cat) {
  const map = new Map();
  const pool = (state.byCategory && state.byCategory.get(cat)) || [];
  pool.forEach((p) => {
...
function applyFilters(list) {
  ...
  // SIM: два независимых фильтра, сочетаются по И — можно искать, например,
  // «2 слота» + «Есть поддержка eSIM» (региональные версии Samsung .../DS).
  if (f.simSlots && f.simSlots.length) {
    list = list.filter((p) => f.simSlots.includes(simInfo(p.sim).slots));
  }
  if (f.simEsim) {
    list = list.filter((p) => simInfo(p.sim).esim);
  }
  // Состояние: «new» означает «без поля condition», а не condition === "new".
  if (f.condition === "new") list = list.filter((p) => !p.condition);
  else if (f.condition) list = list.filter((p) => p.condition === f.condition);
  if (state.favOnly) {
...
function renderCategoryTiles() {
  ...
  const count = (key) => ((state.byCategory && state.byCategory.get(key)) || []).length;
  grid.innerHTML = cats
    .map(
      (c) => `
    <button class="cat-tile" data-cat-tile="${esc(c.key)}" style="--accent:${c.accent || "#0582F6"}">
      <div class="${c.image ? "grid place-items-center aspect-square rounded-2xl media-plate overflow-hidden" : "cat-tile-art"}" data-glyph="${esc(c.glyph || "accessory")}" data-glyph-size="120">${c.image ? `<img src="${esc(c.image)}" data-src="${esc(c.image)}" alt="${esc(c.title)}" class="cat-img h-full w-full object-contain p-4 sm:p-6" loading="lazy" decoding="async" onerror="imgRetry(this)">` : deviceGlyph({ category: c.glyph || "accessory", colorHex: "#ffffff" }, 120)}</div>
    ...
    .join("");
  if ($("#result-count")) $("#result-count").textContent = `${cats.length} категорий`;
...
function renderSeriesTiles() {
  ...
  let series = seriesOfCategory(state.category)
    .map((s) => ({ ...s, filtered: applyFilters(s.items), from: Math.min(...applyFilters(s.items).filter(hasPrice).map((p) => p.priceCash)) }))
  ...
  grid.innerHTML = series
    ...
    <button class="card text-left" data-series-tile="${esc(s.series)}">
      <div class="${(Array.isArray(rep.images) && rep.images.length) ? "media-plate" : "series-art"} h-40 rounded-2xl overflow-hidden grid place-items-center" style="--accent:${accent}">${pmedia(rep, 96, i < 3)}</div>
      ...
      <div class="mt-3 text-sm">${Number.isFinite(from) ? `<span class="text-ink-mute">от</span> <span class="font-semibold text-base">${fmt(from)} ${state.currency}</span>` : `<span class="font-semibold text-base">${PRICE_ON_REQUEST}</span>`}</div>
    </button>`;
    })
    .join("");
...
function renderModifications() {
  ...
  if (!grid) return;
  grid.className = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5";
  let list = sortList(applyFilters(state.all.filter((p) => p.series === state.series && (window.catOf ? window.catOf(p) : p.category) === state.category)));
  ...
  grid.innerHTML = cards.map((c, i) => productCard(c.rep, c.variants, i < 3)).join("");
...
function renderFilters() {
  ...
  // Контекст: если выбрана модель (Поколение) — цвет/память/SIM показываем только
  // те, что реально есть у этой модели (иначе список цветов был бы «глобальным»).
  const ctxPool = state.filters.series ? pool.filter((p) => p.series === state.filters.series) : pool;
  const isWatch = state.category === "Смарт-часы";
  const defs = [
    { key: "series", label: "Поколение", values: seriesOfCategory(state.category).map((s) => s.series) },
    !isWatch && { key: "storage", label: "Память", values: uniqStorages(ctxPool) },
    isWatch && { key: "watchSize", label: "Размер корпуса", values: uniqWatchSizes(ctxPool) },
    { key: "color", label: "Цвет", values: uniq(ctxPool, "color") },
  ...
  const select = (key, label, values) => `<label class="relative">
      <select class="field appearance-none pr-9 text-sm py-2.5" data-filter="${key}">
        ...
        ${values.map((o) => `<option value="${esc(o)}" ${state.filters[key] === o ? "selected" : ""}>${esc(o)}</option>`).join("")}
      </select>${chev}</label>`;

  // ---- SIM: два независимых чекбокс-фильтра вместо одного select ----
  // «Количество физических слотов» (1 слот / 2 слота) и «Есть поддержка eSIM» —
  // ортогональные признаки, сочетаются по И. Так можно найти, например, региональные
  // версии Samsung .../DS: 2 слота + eSIM → «2x Nano-SIM + eSIM».
  const ctxSim = ctxPool.map((p) => simInfo(p.sim));
  const hasSlot1 = ctxSim.some((s) => s.slots === 1);
  const hasSlot2 = ctxSim.some((s) => s.slots === 2);
  const hasEsim = ctxSim.some((s) => s.esim);
  const simToggle = (checked, attr, text) => `
      <label class="btn-pill text-sm inline-flex items-center gap-1.5 cursor-pointer select-none ${checked ? "is-active" : ""}">
        <input type="checkbox" class="w-3.5 h-3.5 accent-brand" ${attr} ${checked ? "checked" : ""} />
        <span>${text}</span>
      </label>`;
  // ---- Состояние: показываем фильтр только когда в разделе реально есть б/у
  // или витринные позиции. В магазине, где всё новое, лишний select только
  // мешает.
  let condBlock = "";
  const condVals = [...new Set(ctxPool.map((p) => p.condition).filter(Boolean))];
  if (condVals.length) {
    const opts = [{ v: "new", t: "Новые" }]
      .concat(condVals.map((v) => ({ v, t: (window.CONDITIONS.find((c) => c.v === v) || {}).t || v })));
    condBlock = `<label class="relative">
      <select class="field appearance-none pr-9 text-sm py-2.5" data-filter="condition">
        <option value="">Состояние: все</option>
        ${opts.map((o) => `<option value="${esc(o.v)}" ${state.filters.condition === o.v ? "selected" : ""}>${esc(o.t)}</option>`).join("")}
      </select>${chev}</label>`;
  }

  let simBlock = "";
  if (hasSlot1 || hasSlot2 || hasEsim) {
    simBlock = `<div class="flex items-center gap-1.5 flex-wrap" data-sim-group>
      <span class="text-xs text-ink-mute mr-0.5">SIM:</span>
      ${hasSlot1 ? simToggle(state.filters.simSlots.includes(1), 'data-sim-slot="1"', "1 слот") : ""}
      ${hasSlot2 ? simToggle(state.filters.simSlots.includes(2), 'data-sim-slot="2"', "2 слота") : ""}
      ${hasEsim ? simToggle(state.filters.simEsim, "data-sim-esim", "Есть поддержка eSIM") : ""}
    </div>`;
  }
  ...
  wrap.innerHTML = defs.map((d) => select(d.key, d.label, d.values)).join("") + condBlock + simBlock + sortSel;
  ...
  $$("[data-filter]", wrap).forEach((sel) =>
    sel.addEventListener("change", () => {
      ...
      if (sel.dataset.filter === "series") {
        // Смена модели меняет доступные цвета/память/SIM — сбрасываем зависимые
        // фильтры и перерисовываем панель, чтобы не осталось «чужих» значений.
        state.filters.color = "";
        state.filters.storage = "";
        state.filters.watchSize = "";
        state.filters.simSlots = [];
        state.filters.simEsim = false;
        renderFilters();
      }
      rerenderGrid();
    })
  );
  $$("[data-sim-slot]", wrap).forEach((cb) =>
    cb.addEventListener("change", () => {
      const n = parseInt(cb.dataset.simSlot, 10);
      const set = new Set(state.filters.simSlots);
      if (cb.checked) set.add(n); else set.delete(n);
      state.filters.simSlots = [...set];
      cb.closest("label")?.classList.toggle("is-active", cb.checked);
      rerenderGrid();
    })
  );
  $("[data-sim-esim]", wrap)?.addEventListener("change", (e) => {
    state.filters.simEsim = e.target.checked;
    e.target.closest("label")?.classList.toggle("is-active", e.target.checked);
    rerenderGrid();
  });
...
function productLink(p) {
  // На боевом (http/https) — чистый пререндер-URL /p/<id>/ (SEO + красивый адрес).
  // Локально (file://) чистые URL не резолвятся — используем product.html?id=.
...
// Канонизация типа SIM: схлопывает пробелы/синонимы, чтобы в конфигураторе не было
// дублей вроде «2 eSIM / 2 eSIM». Итоговые категории: «SIM + eSIM» / «SIM» / «eSIM» /
// «2 eSIM» / «2 SIM» / «2 SIM + eSIM» (см. подробный комментарий в config.js —
// функция здесь держится идентичной, т.к. app.js подключается после config.js и
// переопределяет window.normSim/window.simInfo своей копией).
function normSim(s) {
  if (s == null) return "";
  const v = String(s).replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();
  const low = v.toLowerCase();
  if (low === "" || low === "—" || low === "-" || low === "нет") return "";
  // 2 физических слота (Nano-SIM) + eSIM — проверяем ПЕРВЫМ (иначе совпало бы как
  // «SIM + eSIM» одиночный, или как «2 eSIM» — два eSIM без физических слотов).
...
if (typeof window !== "undefined") window.normSim = normSim;

// Разбор канонической метки SIM на два независимых признака — количество физических
// слотов и поддержку eSIM. Используется фильтрами каталога (два чекбокса вместо
// одного select) и конфигуратором товара.
function simInfo(s) {
  const v = normSim(s);
  switch (v) {
...
if (typeof window !== "undefined") window.simInfo = simInfo;
...
function productFrames(p, size = 120, thumb = false, defer = false, eager = false) {
  const arr = (thumb && Array.isArray(p.thumbs) && p.thumbs.length)
    ? p.thumbs
    : (Array.isArray(p.images) && p.images.length ? p.images : null);
  if (arr) {
    return arr.map((src, i) => {
      const full = (thumb && Array.isArray(p.images) && p.images[i]) ? p.images[i] : src;
      const fb = full !== src ? ` onerror="this.onerror=null;this.src='${esc(full)}'"` : "";
      // defer: грузим только первый кадр; остальные — по требованию (скрабер/свайп)
      const srcAttr = (defer && i > 0) ? `data-src="${esc(src)}"` : `src="${esc(src)}"`;
      // LCP: первый кадр верхних карточек грузим приоритетно и синхронно
      const prio = (eager && i === 0)
        ? `loading="eager" fetchpriority="high" decoding="sync"`
        : `loading="lazy" decoding="async"`;
      return `<img ${srcAttr} alt="${esc(p.name)}" class="max-h-[86%] max-w-[86%] object-contain" ${prio}${fb}>`;
    });
  }
...
// Медиа для плиток/поиска: лёгкое превью (thumb), иначе полное фото, иначе глиф
function pmedia(p, size = 96, eager = false) {
  const src = (Array.isArray(p.thumbs) && p.thumbs[0]) || (Array.isArray(p.images) && p.images[0]);
  if (src) {
    const full = Array.isArray(p.images) && p.images[0];
    const fb = full && full !== src ? ` onerror="this.onerror=null;this.src='${esc(full)}'"` : "";
    const prio = eager ? `loading="eager" fetchpriority="high" decoding="sync"` : `loading="lazy" decoding="async"`;
    return `<img src="${esc(src)}" alt="${esc(p.name)}" class="max-h-[88%] max-w-[88%] object-contain" ${prio}${fb}>`;
  }
  return deviceGlyph(p, size);
}

function cardMedia(p, eager = false) {
  const frames = productFrames(p, 120, true, true, eager); // превью + отложенная загрузка кадров
  return `
    <div class="card-media relative block h-44 rounded-2xl overflow-hidden media-plate" data-media>
...
function colorSwatches(rep, variants) {
  ...
  return `<div class="flex items-center gap-1.5 mt-3 relative z-10" data-swatches>
    ${cols
      .map(
        (v) =>
...
// Метка состояния рядом со статусом наличия. Для новых позиций ничего не
// рисуем: «Новое» на каждой из 274 карточек — шум.
function condChip(p) {
  const t = window.conditionLabel ? window.conditionLabel(p) : "";
  return t ? `<span class="chip bg-ink/[0.06] text-ink-soft shrink-0">${esc(t)}</span>` : "";
}

function productCard(p, variants = [p], eager = false) {
  ...
  const badge = p.badge
    ...
    : "";
  return `
  <article class="card group animate-fade-up" data-id="${p.id}" data-model="${esc(p.name)}">
    <a href="${productLink(p)}" class="absolute inset-0 z-[1] rounded-3xl" aria-label="${esc(p.name)}" data-cardlink></a>
    ${badge}
    ...
    <div class="mt-3 flex flex-wrap items-center gap-1.5" data-status>${statusBadge(p.status)}${condChip(p)}</div>

    <div class="mt-4">
      <div class="text-xl font-semibold tracking-tight" data-cash>${priceText(p)}</div>
      ${p.priceEstimated ? `<div class="text-[11px] text-ink-mute mt-0.5">цена ориентировочная</div>` : ""}
    </div>

    <button class="btn-primary w-full mt-4 relative z-10" data-buy="${p.id}">Оформить</button>
  </article>`;
}

function pickRep(variants) {
  if (!variants || !variants.length) return variants && variants[0];
  const hasImg = (v) => Array.isArray(v.images) && v.images.length;
  const key = variants[0].category + "||" + variants[0].name;
  // 1) Явно заданная обложка модели (цвет) — из meta.covers, задаётся в админке.
  const coverColor = state.covers && state.covers[key];
  if (coverColor) {
    const byColor = variants.filter((v) => v.color === coverColor);
    const withImg = byColor.filter(hasImg);
    if (withImg.length) return withImg[0];
    if (byColor.length) return byColor[0];
  }
  // 2) Иначе: самый дешёвый В НАЛИЧИИ среди тех, у кого ЕСТЬ фото (чтобы плитка
  //    не показывала цвет без снимка, напр. «Облачный белый» без фото).
  const inStock = variants.filter((v) => v.status === "in_stock");
  const pool = inStock.length ? inStock : variants;
  const withImg = pool.filter(hasImg);
  const finalPool = withImg.length ? withImg : pool;
  return finalPool.slice().sort((a, b) => a.priceCash - b.priceCash)[0];
}
...
// Цена к показу. Ноль означает «цены пока нет» — так заводятся комплектации,
// которых нет в прайсе поставщика. Показывать «0 ₽» нельзя: покупатель решит,
// что товар бесплатный, и нажмёт «Оформить». Такие позиции не попадают ни в
// товарные фиды, ни в микроразметку цены.
const PRICE_ON_REQUEST = "Уточняйте у менеджера";
function hasPrice(p) { return Number(p && p.priceCash) > 0; }
function priceText(p) {
  return hasPrice(p) ? `${fmt(p.priceCash)} ${state.currency}` : PRICE_ON_REQUEST;
}
...
function wireCard(article) {
  ...
  // Навигация по карточке — через настоящий <a data-cardlink> (нативный контекст-меню,
  // «открыть в новой вкладке», средний клик). Клик по кнопкам/свотчам не должен вести на товар.
}
...
function setupScrubber(media) {
  if (!media) return;
  media.style.touchAction = "pan-y"; // вертикальный скролл страницы не блокируем
  ...
  let cur = 0;
  const load = (i) => {
    const img = frames[i] && frames[i].querySelector("img[data-src]");
  ...
  // ДЕСКТОП (мышь) — скраб при наведении; МОБИЛЬНЫЙ (touch) — свайп влево/вправо
  ...
  media.addEventListener("pointerdown", (e) => {
    if (e.pointerType !== "mouse") { swiping = true; swiped = false; sx = e.clientX; sy = e.clientY; }
  });
  ...
  // если был горизонтальный свайп — не переходим по карточке
  media.addEventListener("click", (e) => { if (swiped) { e.stopPropagation(); e.preventDefault(); swiped = false; } }, true);
}
...
function searchSuggestions() {
  const popular = ["iPhone 17 Pro", "MacBook M4", "AirPods Pro 3", "iPhone 17", "iPad Air", "Apple Watch"];
...
function setupSearch() {
  ...
  const render = () => {
    ...
    box.innerHTML = hits
      ...
      <a class="flex w-full items-center gap-3 p-3 rounded-2xl hover:bg-black/[0.04] transition" href="${productLink(p)}">
        <span class="grid place-items-center w-11 h-11 rounded-xl media-plate shrink-0 overflow-hidden">${pmedia(p, 30)}</span>
        ...
        <span class="text-sm font-semibold whitespace-nowrap">${priceText(p)}</span>
      </a>`
      )
      .join("");
  };
  ...
  input.addEventListener("input", debounce(render, 150)); // антидребезг ввода
  ...
  root.innerHTML = `
    <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" data-overlay>
      ...
      <div class="glass relative w-full ${opts.wide ? "sm:max-w-4xl" : "sm:max-w-lg"} max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl shadow-pop animate-scale-in">
        <button class="absolute right-4 top-4 z-10 grid place-items-center w-9 h-9 rounded-full bg-black/5 hover:bg-black/10 transition text-ink-soft" data-close aria-label="Закрыть"><svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
...
function openLeadModal(productId, presetText = "") {
  ...
  const sub = p
    ? `${[p.storage, p.color].filter((x) => x && x !== "—").join(" · ")} · ${priceText(p)}`
  ...
  const html = `
    ...
    <form id="lead-form" class="mt-6 space-y-4" novalidate>
      <input class="field" name="name" placeholder="Как к вам обращаться (необязательно)" autocomplete="name" />
      ...
      if (container) {
        try {
          turnstileWidgetId = turnstile.render(container, {
            ...
            // Без этих колбэков провал капчи был невидим: виджет молчал, а
            // заявка отбивалась воркером с captcha_failed уже после нажатия
            // «Отправить». Теперь причина видна сразу.
            "error-callback": (code) => {
              console.warn("Turnstile error:", code);
              container.innerHTML =
                '<p class="text-xs text-center text-apple-red">Проверка не загрузилась. ' +
                'Обновите страницу или напишите нам в Telegram.</p>';
            },
            "expired-callback": () => { try { turnstile.reset(turnstileWidgetId); } catch (e) {} },
          });
        } catch (err) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    // LOADING: блокируем повторную отправку, показываем прогресс
    if (submitBtn) { submitBtn.disabled = true; submitBtn.classList.add("opacity-70", "pointer-events-none"); submitBtn.textContent = "Отправляем…"; }
    status.innerHTML = "";
    status.textContent = "Отправляем заявку…";
    status.className = "text-sm text-center text-ink-mute";

    const restoreBtn = () => {
      if (submitBtn) { submitBtn.disabled = false; submitBtn.classList.remove("opacity-70", "pointer-events-none"); submitBtn.textContent = submitLabel; }
    };
    ...
    const payload = {
      ...
      product: p
        ? [p.name, (p.storage && p.storage !== "—") ? p.storage : (p.watchSize || null), p.color, (p.sim && p.sim !== "—") ? (window.simLabel ? window.simLabel(p.sim) : p.sim) : null]
            .filter(Boolean).join(" · ")
        : "—",
      price: p ? `${priceText(p)}` : "—",
      page: location.href,
    ...
    try {
      ...
      // Цель Метрики. Вызов безопасен, если счётчика нет: ym не определён и
      // условие просто не выполнится.
      ymGoal("lead_sent", { product: payload.product, price: payload.price });
      form.reset();
      ...
      // FAILURE: понятное сообщение + «Попробовать снова» + прямые ссылки на мессенджеры
      restoreBtn();
      const waText = encodeURIComponent(`Заявка с сайта\nТовар: ${payload.product}\nИмя: ${payload.name}\nТелефон: ${payload.phone}`);
      status.className = "text-sm text-center";
      status.innerHTML =
        `<span class="text-apple-red">Не удалось отправить заявку.</span>` +
        `<span class="block text-ink-mute mt-0.5">Проверьте соединение или свяжитесь с нами напрямую.</span>` +
        `<span class="mt-2 flex items-center justify-center gap-2">` +
        `<button type="button" data-lead-retry class="btn-soft text-sm">Попробовать снова</button>` +
        `<a href="${esc(CONFIG.contacts.telegram)}" target="_blank" rel="noopener" class="btn-pill text-sm">Telegram</a>` +
        `<a href="${esc(CONFIG.contacts.whatsapp || CONFIG.contacts.telegram)}?text=${waText}" target="_blank" rel="noopener" class="btn-pill text-sm">WhatsApp</a>` +
        `</span>`;
      status.querySelector("[data-lead-retry]")?.addEventListener("click", () => form.requestSubmit ? form.requestSubmit() : form.dispatchEvent(new Event("submit", { cancelable: true })));
...
// ---- Сравнение (FR-1.3) ----
// Ленивая подгрузка классического скрипта по требованию (аналог dynamic import,
// но работает и на file://). Второстепенные модули (compare/trade-in) не грузятся,
// пока пользователь не откроет соответствующий блок.
const __loadedScripts = {};
function ensureScript(src) {
  return new Promise((resolve, reject) => {
    if (__loadedScripts[src]) return resolve();
    const s = document.createElement("script");
    s.src = src;
    s.onload = () => { __loadedScripts[src] = true; resolve(); };
...
async function openCompareModal() {
  try { await ensureScript("./js/compare.js"); } catch (e) { return; }
  const html = `
...
async function openTradeInModal() {
  try { await ensureScript("./js/trade-in.js"); } catch (e) { return; }
  ...
  const html = `
    ...
    <p class="mt-4 text-xs text-ink-mute leading-relaxed flex items-start gap-1.5">
      <svg class="w-4 h-4 shrink-0 text-apple-amber mt-px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><path d="M12 9v4M12 17h.01"/></svg>
      <span>Итоговая сумма выкупа определяется <b>после бесплатной диагностики устройства в магазине</b>.
      Оценка на сайте является предварительной и не является публичной офертой.</span>
    </p>
...
function openCallbackModal() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const submitLabel = submitBtn ? submitBtn.textContent : "";
    if (submitBtn) { submitBtn.disabled = true; submitBtn.classList.add("opacity-70", "pointer-events-none"); submitBtn.textContent = "Отправляем…"; }
    const restoreBtn = () => { if (submitBtn) { submitBtn.disabled = false; submitBtn.classList.remove("opacity-70", "pointer-events-none"); submitBtn.textContent = submitLabel; } };
    status.textContent = "Отправляем заявку…"; status.className = "text-sm text-center text-ink-mute";
    ...
    try {
      ...
      status.className = "text-sm text-center";
      status.innerHTML =
        `<span class="text-apple-red">Не удалось отправить.</span>` +
        `<span class="mt-2 flex items-center justify-center gap-2">` +
        `<button type="button" data-lead-retry class="btn-soft text-sm">Попробовать снова</button>` +
        `<a href="${esc(CONFIG.contacts.telegram)}" target="_blank" rel="noopener" class="btn-pill text-sm">Telegram</a>` +
        `</span>`;
...
function applyTheme(theme) {
  ...
  try { localStorage.setItem("ait-theme", dark ? "dark" : "light"); } catch (e) {}
  const meta = document.querySelector('meta[name="theme-color"]');
  if (meta) meta.setAttribute("content", dark ? "#111113" : "#FAF9F6");
  $$("[data-theme-toggle]").forEach((btn) => {
    const sun = btn.querySelector("[data-theme-sun]");
    const moon = btn.querySelector("[data-theme-moon]");
    if (sun) sun.classList.toggle("hidden", dark);   // в тёмной теме показываем луну
...
async function init() {
  ...
  // ПЕРВЫЙ РЕНДЕР — мгновенно. Источник: кэш сессии (самый свежий каталог,
  // полученный из KV ранее в этой вкладке), иначе встроенный каталог. Никакого
  // ожидания сети → нет «скачка» макета (CLS) и мгновенный показ, в т.ч. при
  // возврате со страницы товара.
  const embedded = window.__CATALOG__ || { products: [], categories: [] };
  const cached = readCatalogCache();
  const initial = cached || embedded;
  ...
  // Открыли сразу раздел каталога (#cat=…): после перезагрузки вкладки или
  // возврата из мессенджера страница стояла на главном экране, а потом браузер
  // рывком прокручивал её вниз. Встаём на каталог сразу, без анимации, — но
  // только если браузер сам не восстановил позицию прокрутки.
  if (state.level > 1) {
    const toCatalog = () => { if (window.scrollY < 50) $("#catalog")?.scrollIntoView({ block: "start" }); };
    requestAnimationFrame(toCatalog);
    window.addEventListener("load", toCatalog, { once: true });
  }

  // ФОНОВОЕ обновление из KV (реальные цены/остатки). Перерисовываем и обновляем
  // кэш ТОЛЬКО если данные реально изменились — иначе лишнего сдвига/работы нет.
  if (window.CONFIG && CONFIG.apiBase) {
    fetch(`${CONFIG.apiBase}/api/products`, { cache: "no-cache" })
      .then((r) => (r.ok ? r.json() : null))
      .then((fresh) => {
        if (!fresh || !Array.isArray(fresh.products)) return;
        const freshSig = JSON.stringify((fresh.products || []).filter((p) => !p.hidden));
        if (freshSig !== state.rawSig) {
          applyCatalog(fresh);
          renderCatalog();
        }
  ...
  // Онлайн-калькулятор Trade-In ВРЕМЕННО ОТКЛЮЧЁН: формула считала выкуп от
  // базовой цены по модели и памяти, не учитывая реальное состояние устройства,
  // и расходилась с оценкой в магазине. Показать покупателю сумму, которую
  // потом придётся пересматривать вниз, хуже, чем не показывать никакой.
  // Модуль trade-in.js и openTradeInModal оставлены в коде: чтобы вернуть,
  // достаточно поставить CONFIG.tradeInCalculator = true и вернуть кнопки
  // [data-open-tradein] в index.html.
  if (CONFIG.tradeInCalculator === true) {
...
// ==== SEO: canonical + Schema.org JSON-LD ============================
...
if (typeof window !== "undefined") { window.setCanonical = setCanonical; window.setJsonLd = setJsonLd; }

function renderSeoHome() {
  const C = window.CONFIG || {};
  const base = (C.siteUrl || location.origin || "").replace(/\/$/, "");
  setCanonical(base + "/");
  const loc = (Array.isArray(C.locations) && C.locations[0]) || null;
  const rev = (C.reviews && Array.isArray(C.reviews.sources) && C.reviews.sources[0]) || null;
  const sameAs = C.contacts ? [C.contacts.telegram, C.contacts.vk, C.contacts.whatsapp].filter(Boolean) : [];
  const ld = {
    "@context": "https://schema.org",
    "@type": "Store",
    name: C.brand || "Apple и точка",
    url: base + "/",
    image: base + "/apple-touch-icon.png",
    telephone: C.contacts && C.contacts.phone,
    areaServed: C.city,
    priceRange: "₽₽",
    sameAs,
  };
  if (loc) ld.address = { "@type": "PostalAddress", streetAddress: loc.address || "", addressLocality: C.city || "", addressCountry: "RU" };
  if (loc && Number.isFinite(loc.lat) && Number.isFinite(loc.lon)) ld.geo = { "@type": "GeoCoordinates", latitude: loc.lat, longitude: loc.lon };
  if (rev && rev.rating) {
    const val = String(rev.rating).replace(",", ".");
    const cnt = parseInt(String(rev.count).replace(/\D/g, ""), 10) || undefined;
    if (cnt) ld.aggregateRating = { "@type": "AggregateRating", ratingValue: val, bestRating: "5", reviewCount: cnt };
  }
  setJsonLd("ld-store", ld);

  // WebSite (для поиска по сайту и AI-краулеров)
  setJsonLd("ld-website", {
    ...
    url: base + "/",
    inLanguage: "ru-RU",
    publisher: { "@type": "Organization", name: C.brand || "Apple и точка", url: base + "/" },
  });
}

// ==== FAQ (видимый блок + FAQPage JSON-LD: SEO rich-results и AI-ответы) ====
const FAQ_ITEMS = [
  { q: "Техника оригинальная?", a: "Да. Мы продаём только оригинальную технику Apple и смежных брендов с гарантией магазина. Не является официальным сайтом Apple Inc." },
  { q: "Как работает Trade-In?", a: "Вы сдаёте старое устройство в счёт нового. Оценка бесплатная: смотрим устройство при вас в магазине и сразу называем сумму выкупа, она вычитается из цены нового." },
  { q: "Можно купить в кредит?", a: "Да, оформляем кредит через банк-партнёр. Сумму, ставку, срок и итоговую переплату назовёт оператор при оформлении — условия зависят от решения банка. Рассрочки у нас нет." },
  { q: "Какая гарантия?", a: "Гарантия магазина: 12 месяцев на новые смартфоны, планшеты, ноутбуки и часы, 3 месяца на наушники и на технику, бывшую в употреблении." },
  { q: "Доставка и сроки?", a: "Товары в наличии доступны сразу, под заказ привозим за 1 день. Есть доставка по Таганрогу и самовывоз из магазина." },
  { q: "Как оформить заказ?", a: "Нажмите «Оформить» в карточке товара и оставьте имя и телефон — оператор свяжется в течение нескольких минут. Также можно написать в Telegram/WhatsApp или позвонить." },
];
function renderFaq() {
  const list = $("#faq-list");
  const section = $("#faq");
  if (!list) return;
  list.innerHTML = FAQ_ITEMS.map((it) => `
    <details class="group rounded-2xl bg-cloud-card shadow-card overflow-hidden">
      <summary class="flex items-center justify-between gap-3 cursor-pointer select-none p-4 sm:p-5 font-medium">
        <span>${esc(it.q)}</span>
        <svg class="w-5 h-5 text-ink-mute shrink-0 transition-transform group-open:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
      </summary>
      <div class="px-4 sm:px-5 pb-4 sm:pb-5 -mt-1 text-ink-soft text-sm leading-relaxed">${esc(it.a)}</div>
    </details>`).join("");
...
// ==== Отзывы («О нас пишут») =========================================
function starRow(n, size = "w-4 h-4") {
  const full = Math.max(0, Math.min(5, Math.round(Number(n) || 0)));
  let out = "";
  for (let i = 0; i < 5; i++) {
    const on = i < full;
    out += `<svg class="${size} ${on ? "text-amber-400" : "text-black/15"}" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l2.9 6.26L22 9.27l-5 4.87L18.18 22 12 18.56 5.82 22 7 14.14l-5-4.87 7.1-1.01L12 2z"/></svg>`;
  }
...
async function renderReviews() {
  ...
  if (!grid) return;
  // Источник данных: сначала пробуем Worker-кэш (/api/reviews) — обновляется без
  // пересборки сайта, затем локальный CONFIG.reviews как fallback.
  let sources = [];
  const cfg = (window.CONFIG && CONFIG.reviews) || null;
  if (window.CONFIG && CONFIG.apiBase) {
    try {
      const r = await fetch(`${CONFIG.apiBase}/api/reviews`, { cache: "no-cache" });
      if (r.ok) { const d = await r.json(); if (d && Array.isArray(d.sources) && d.sources.length) sources = d.sources; }
    } catch (e) { /* оффлайн/недоступно — используем локальные */ }
  }
  if (!sources.length && cfg && Array.isArray(cfg.sources)) sources = cfg.sources;
  if (!sources.length) { section?.classList.add("hidden"); return; }
  section?.classList.remove("hidden");

  const primary = sources.find((s) => s.rating) || sources[0] || {};
  const cards = [];
  sources.forEach((s) => (Array.isArray(s.featured) ? s.featured : []).forEach((f) => cards.push(f)));

  grid.className = ""; // блок сам управляет раскладкой (шапка + горизонтальная лента)

  // Нет карточек, но задан виджет площадки → показываем официальный iframe.
  if (!cards.length) {
    const wid = sources.find((s) => s.widgetUrl);
    if (wid) {
      // Ширину ограничиваем естественной шириной виджета Яндекса (~560px), иначе справа
      // остаётся пустой серый «хвост» iframe. На мобилке — на всю ширину.
      grid.innerHTML = `<div class="rounded-3xl bg-white shadow-card overflow-hidden w-full max-w-xl">
        <iframe src="${esc(wid.widgetUrl)}" title="Отзывы ${esc(wid.name)}" loading="lazy" class="w-full block" style="height:560px;border:0" referrerpolicy="no-referrer-when-downgrade"></iframe>
      </div>`;
  ...
  const header = `
    <div class="flex items-center justify-between gap-3 mb-4">
      <div class="flex items-center gap-3">
        <span class="text-3xl font-semibold tracking-tight">${esc(primary.rating || "5,0")}</span>
        <div>
          ${starRow(String(primary.rating || "5").replace(",", "."), "w-4 h-4")}
          <div class="text-xs text-ink-mute mt-0.5">${esc(primary.name || "Яндекс Карты")}${primary.count ? " · " + esc(primary.count) : ""}</div>
        </div>
      </div>
      <a href="${esc(primary.url || "#")}" target="_blank" rel="noopener" class="btn-soft text-sm shrink-0">Смотреть все отзывы</a>
    </div>`;

  const cardHtml = cards.map((f) => `
    <div class="snap-start shrink-0 w-[280px] sm:w-[340px] rounded-2xl bg-cloud-card shadow-card p-5">
      <div class="flex items-center gap-2">
        <span class="grid place-items-center w-9 h-9 rounded-full bg-brand/10 text-brand text-sm font-semibold shrink-0">${esc((f.author || "•").slice(0, 1))}</span>
        <div class="min-w-0">
          <div class="font-medium text-sm leading-tight truncate">${esc(f.author || "Покупатель")}</div>
          <div class="text-xs text-ink-mute">${esc(f.date || "")}</div>
        </div>
        <span class="ml-auto">${starRow(f.rating || 5, "w-3.5 h-3.5")}</span>
      </div>
      <p class="mt-3 text-sm text-ink-soft leading-relaxed">${esc(f.text || "")}</p>
    </div>`).join("");

  grid.innerHTML = header +
    `<div class="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2 -mx-1 px-1" style="scrollbar-width:thin">${cardHtml}</div>`;
}

// ==== Карта / «Как нас найти» ========================================
function buildYandexMapSrc(locations, mapCfg) {
  if (mapCfg && mapCfg.embedUrl) return mapCfg.embedUrl;
  const z = (mapCfg && mapCfg.zoom) || 16;
  const pts = locations.filter((l) => Number.isFinite(l.lat) && Number.isFinite(l.lon));
  if (pts.length) {
    const center = pts[0];
    const pt = pts.map((l) => `${l.lon},${l.lat},pm2rdm`).join("~");
    return `https://yandex.ru/map-widget/v1/?ll=${center.lon},${center.lat}&z=${z}&pt=${encodeURIComponent(pt)}`;
  }
  // Координат нет — строим карту по адресу (Яндекс сам геокодирует и ставит пин).
  const addr = (locations.find((l) => l.address) || {}).address;
  if (addr) return `https://yandex.ru/map-widget/v1/?text=${encodeURIComponent(addr)}&z=${z}`;
  return "";
}
...
function renderLocations() {
  ...
  const locs = (window.CONFIG && Array.isArray(CONFIG.locations) ? CONFIG.locations : []).filter(Boolean);
  if (!wrap || !locs.length) { section?.classList.add("hidden"); return; }

  const mapSrc = buildYandexMapSrc(locs, (window.CONFIG && CONFIG.map) || {});
  const pinSvg = `<svg class="w-4 h-4 text-brand shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>`;

  const cards = locs
    .map((l) => {
      const q = encodeURIComponent(l.address || l.name || "");
      return `<a href="https://yandex.ru/maps/?text=${q}" target="_blank" rel="noopener"
        class="block rounded-2xl border border-black/[0.06] p-4 hover:border-brand/40 hover:bg-brand/[0.03] transition">
        <div class="flex items-start gap-2">
          ${pinSvg}
          <div class="min-w-0">
            <div class="font-semibold leading-tight">${esc(l.name || "Магазин")}</div>
            <div class="text-sm text-ink-soft mt-0.5">${esc(l.address || "")}</div>
            ${l.hours ? `<div class="text-xs text-ink-mute mt-1">${esc(l.hours)}</div>` : ""}
          </div>
        </div>
      </a>`;
    })
  ...
  const mapHtml = mapSrc
    ? `<iframe src="${esc(mapSrc)}" title="Карта — как нас найти" loading="lazy"
         class="w-full h-full min-h-[320px]" style="border:0" allowfullscreen referrerpolicy="no-referrer-when-downgrade"></iframe>`
    : `<div class="grid place-items-center h-full min-h-[320px] text-ink-mute text-sm p-6 text-center">Укажите координаты магазина в config.js → карта появится здесь</div>`;

  wrap.innerHTML = `<div class="grid md:grid-cols-[1.4fr_1fr]">
    <div class="min-h-[320px] bg-cloud">${mapHtml}</div>
    <div class="p-4 sm:p-5 flex flex-col gap-3 max-h-[460px] overflow-y-auto">${cards}</div>
  </div>`;
}
