...
(function () {
  ...
  function dimsOf(vs) {
    ...
    if (vs.some((v) => v.region)) dims.push("region");
    return dims;
  }
  ...
  function view(p, vs, dims) {
    ...
    const regions = uniqBy(vs.filter((v) => v.region), (v) => v.region).map((v) => v.region);
    ...
    return `
      ...
      <div class="grid lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div>
          ...
          ${btnRow(p, vs, dims, "region", "Регион", regions)}

          <div class="mt-6 rounded-3xl bg-cloud-card shadow-card p-5">
            <div class="flex items-end justify-between gap-4 flex-wrap">
              <div><div class="text-3xl font-semibold tracking-tight" data-cash>${priceText(p.priceCash)}</div></div>
            </div>
  ...
  function setHead(p) {
    ...
    setMeta("description", `${p.series} ${p.color || ""} ${p.storage !== "—" ? p.storage : ""} ${Number(p.priceCash) > 0 ? "— цена от " + money(p.priceCash) : "— " + PRICE_ON_REQUEST}. ${statusText(p.status)}. Trade-In, кредит, гарантия магазина «Apple и точка».`);
  }
