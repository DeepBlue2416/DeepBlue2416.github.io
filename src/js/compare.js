...
// (classic script — Store берётся из глобальной области, storage.js подключается раньше)
...
const Compare = {
  ...
  buildTableHTML(products, currency = "₽") {
    ...
    const baseRows = [
      ...
      ["Память", (p) => p.storage || "—"],
      ["Цвет", (p) => p.color || "—"],
      ["SIM", (p) => (window.simLabel ? window.simLabel(p.sim) : p.sim) || "—"],
      ["Цена", (p) => money(p.priceCash, currency)],
      ["Статус", (p) => (p.status === "in_stock" ? "В наличии" : "Под заказ 1 день")],
    ];
    ...
    const body = `
      <tbody>
        ${baseRows.map(([l, g], i) => rowHTML(l, g, l === "Цена")).join("")}
        ${specKeys
          .map((key) =>
