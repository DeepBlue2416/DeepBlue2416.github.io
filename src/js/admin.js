...
(function () {
  ...
  // Единая цена: поле priceCard больше не используется в UI, храним = priceCash
  // (для обратной совместимости данных). Две цены на сайте убраны.
  // Цена по карте = наличные + 15%. Раньше здесь было Math.round(cash), то есть
  // карта приравнивалась к наличным, хотя комментарии обещали +10% — из-за этого
  // в каталоге накопился разнобой: часть позиций с наценкой, часть без.
  // Значение должно совпадать с CARD_RATIO в worker/tg-price-sync.js.
  const CARD_RATIO = 1.15;
  const cardPrice = (cash) => Math.round(((Number(cash) || 0) * CARD_RATIO) / 10) * 10;

  // ---------- Иконки (инлайн SVG, не зависят от icons.js) ----------
  const P = {
    // eye / eye-off — для кнопок скрытия товара. Добавлены именно сюда:
    // svg() в этом файле читает локальный набор P, и иконки из icons.js
    // здесь не видны (на этом уже получили пустой квадрат вместо глаза).
    eye: '<path d="M2 12s3.6-7 10-7 10 7 10 7-3.6 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/>',
    "eye-off": '<path d="m3 3 18 18"/><path d="M10.6 5.2A9.9 9.9 0 0 1 12 5c6.4 0 10 7 10 7a18 18 0 0 1-2.4 3.3"/><path d="M6.5 6.8A18 18 0 0 0 2 12s3.6 7 10 7c1.4 0 2.7-.3 3.8-.8"/><path d="M9.9 9.9a3 3 0 0 0 4.2 4.2"/>',
  ...
  // Состояние устройства. Берём список из config.js, чтобы витрина и CRM не
  // разъезжались; если config почему-то не загрузился — свой запасной набор.
  const CONDITION_OPTS = (window.CONDITIONS) || [
    { v: "", t: "Новое" },
    { v: "used", t: "Б/у" },
    { v: "demo", t: "Витринный" },
  ];
  const condTitle = (v) => (CONDITION_OPTS.find((c) => c.v === v) || {}).t || v;
  // Канонический набор типов SIM (без синонимов, чтобы не было дублей в списке).
  // «2 SIM» / «2 SIM + eSIM» — два физических Nano-SIM слота (второе — ещё и с
  // поддержкой eSIM, как у региональных версий Samsung .../DS).
  const SIM_BASE = ["SIM + eSIM", "SIM", "eSIM", "2 eSIM", "2 SIM", "2 SIM + eSIM", "—"];
  const normSimAdm = (window.normSim) || ((s) => s);
  ...
  function simOptions(current) {
    ...
    const push = (v) => {
      if (v == null || v === "") return;
      if (v === "—") { if (!out.includes("—")) out.push("—"); return; }
      const n = normSimAdm(v) || v;              // канонизируем — убираем дубли/синонимы
  ...
  // Регион поставки (для грей-маркет модификаций, напр. Samsung .../DS) — чисто
  // информационное поле; влияет только на отображение (доп. измерение в матрице
  // товара, строка «Регион» в характеристиках, бейдж на карточке).
  const REGION_BASE = ["🇦🇪 ОАЭ", "🇰🇿 Казахстан", "🇹🇭 Таиланд"];
  ...
  const state = {
    ...
    redo: [],
    tab: "products",
    reviews: null, // витрина отзывов {sources:[...]} — грузится при открытии вкладки
    query: "",
    filterCat: "all",
  ...
  function markDirty() {
    ...
    const btn = $("#btn-save");
    btn.disabled = !has;
    // Появились новые правки после ошибки — сбрасываем «красное» состояние кнопки
    if (has && btn.classList.contains("bg-apple-red")) {
      btn.classList.remove("bg-apple-red", "hover:bg-apple-red", "opacity-80");
  ...
  async function loadCatalog() {
    ...
    if (state.api) {
      try {
        // includeHidden=1 — иначе скрытые позиции не видны в CRM и их
        // невозможно вернуть в продажу: витрина и этот эндпоинт по умолчанию
        // их вырезают.
        data = await api("/api/products?includeHidden=1");
      } catch (e) {
  ...
  async function save() {
    ...
    btn.disabled = true;
    btn.classList.remove("bg-apple-red", "hover:bg-apple-red");
    ...
    try {
      ...
      state.redo.length = 0;
      markDirty();
      // SUCCESS: краткая отметка «Сохранено ✓», затем обычное состояние
      btn.classList.remove("opacity-80");
      btn.textContent = "Сохранено ✓";
      toast("Сохранено в KV. Резервная копия создана.");
      setTimeout(() => { btn.textContent = "Сохранить"; markDirty(); }, 1400);
    } catch (e) {
      // FAILURE: явное состояние ошибки прямо на кнопке + возможность повторить
      btn.classList.remove("opacity-80");
      btn.classList.add("bg-apple-red", "hover:bg-apple-red");
      btn.textContent = "Ошибка — повторить";
      btn.disabled = false;
      toast("Не сохранено: " + (e && e.message ? e.message : "ошибка сети"));
    }
  }
  ...
  function newId(p) {
    // Префикс — категория, как у всех существующих позиций каталога
    // (iphone-iphone-17-pro-…). Раньше новые товары получали префикс «item» и
    // выбивались из общего вида адресов /p/<id>/.
    const parts = [p.category, p.name, p.color, p.storage || p.watchSize, p.sim];
    if (p.condition) parts.push(p.condition === "used" ? "bu" : p.condition);
    if (p.region) parts.push(p.region); // регион добавляем в slug только если он задан
    const base = parts.map(slug).join("-");
  ...
  function renderActive() {
    ...
    else if (state.tab === "reviews") renderReviewsTab();
  ...
  function switchTab(tab) {
    ...
    $("#tab-reviews").classList.toggle("hidden", tab !== "reviews");
  ...
  // ---------- Отзывы (витрина «О нас пишут») ----------
  const $$a = (s, r = document) => Array.from(r.querySelectorAll(s));
  function cfgReviewSources() {
    return (window.CONFIG && CONFIG.reviews && Array.isArray(CONFIG.reviews.sources)) ? clone(CONFIG.reviews.sources) : [];
  }
  function loadReviews() {
    if (state.reviews) return renderReviewsTab();
    const done = (sources) => { state.reviews = { sources: sources || [] }; renderReviewsTab(); };
    if (state.api) {
      fetch(`${state.api}/api/reviews`, { cache: "no-cache" })
        .then((r) => (r.ok ? r.json() : null))
        .then((d) => done((d && Array.isArray(d.sources) && d.sources.length) ? d.sources : cfgReviewSources()))
        .catch(() => done(cfgReviewSources()));
    } else {
      done(cfgReviewSources());
    }
  }
  function blankSource() { return { name: "Новый источник", url: "", widgetUrl: "", rating: "5,0", count: "0 отзывов", featured: [] }; }
  function blankReview() { return { author: "", date: "", rating: 5, text: "" }; }

  function renderReviewsTab() {
    const wrap = $("#rv-list");
    if (!wrap) return;
    const srcs = (state.reviews && state.reviews.sources) || [];
    if (!srcs.length) { wrap.innerHTML = `<div class="text-sm text-ink-mute">Пока нет источников. Нажмите «Источник», чтобы добавить Яндекс или 2ГИС.</div>`; return; }
    wrap.innerHTML = srcs.map((s, si) => {
      const feats = Array.isArray(s.featured) ? s.featured : [];
      const featRows = feats.map((f, fi) => `
        <div class="rounded-xl border border-black/[0.06] p-3 mt-2" data-fi="${fi}">
          <div class="flex items-center gap-2">
            <input class="adm-input flex-1" data-si="${si}" data-fi="${fi}" data-field="author" value="${esc(f.author || "")}" placeholder="Имя автора"/>
            <input class="adm-input w-28" data-si="${si}" data-fi="${fi}" data-field="date" value="${esc(f.date || "")}" placeholder="дата"/>
            <select class="adm-input w-16" data-si="${si}" data-fi="${fi}" data-field="rating">${[5,4,3,2,1].map((n)=>`<option value="${n}" ${Number(f.rating)===n?"selected":""}>${n}★</option>`).join("")}</select>
            <button class="adm-icbtn text-apple-red" data-rv-act="del-review" data-si="${si}" data-fi="${fi}" title="Удалить отзыв">${svg("trash","w-4 h-4")}</button>
          </div>
          <textarea class="adm-input w-full mt-2" rows="2" data-si="${si}" data-fi="${fi}" data-field="text" placeholder="Текст отзыва">${esc(f.text || "")}</textarea>
        </div>`).join("");
      return `<div class="bg-cloud-card rounded-2xl border border-black/[0.06] shadow-sm p-4" data-si="${si}">
        <div class="flex items-center gap-2">
          <input class="adm-input font-semibold flex-1" data-si="${si}" data-field="name" value="${esc(s.name || "")}" placeholder="Название источника (Яндекс Карты / 2ГИС)"/>
          <button class="adm-icbtn text-apple-red" data-rv-act="del-src" data-si="${si}" title="Удалить источник">${svg("trash","w-4 h-4")}</button>
        </div>
        <div class="grid sm:grid-cols-2 gap-2 mt-2">
          <label class="block"><span class="adm-label">Ссылка на карточку</span><input class="adm-input" data-si="${si}" data-field="url" value="${esc(s.url || "")}" placeholder="https://yandex.ru/maps/org/…"/></label>
          <label class="block"><span class="adm-label">Виджет (iframe URL) — «живые» отзывы</span><input class="adm-input" data-si="${si}" data-field="widgetUrl" value="${esc(s.widgetUrl || "")}" placeholder="https://yandex.ru/maps-reviews-widget/…"/></label>
          <label class="block"><span class="adm-label">Рейтинг</span><input class="adm-input" data-si="${si}" data-field="rating" value="${esc(s.rating || "")}" placeholder="5,0"/></label>
          <label class="block"><span class="adm-label">Число отзывов (подпись)</span><input class="adm-input" data-si="${si}" data-field="count" value="${esc(s.count || "")}" placeholder="120 отзывов"/></label>
        </div>
        <div class="mt-3 flex items-center justify-between">
          <span class="text-xs text-ink-mute">Избранные отзывы (${feats.length})</span>
          <button class="btn-pill text-xs" data-rv-act="add-review" data-si="${si}">+ отзыв</button>
        </div>
        ${featRows}
      </div>`;
    }).join("");
    ...
    // Правки значений
    $$a("[data-field]", wrap).forEach((el) => {
      el.addEventListener("input", () => {
        const si = +el.dataset.si, fi = el.dataset.fi != null ? +el.dataset.fi : null, field = el.dataset.field;
        const s = state.reviews.sources[si]; if (!s) return;
        if (fi == null) { s[field] = el.value; }
        else { s.featured = s.featured || []; if (s.featured[fi]) s.featured[fi][field] = field === "rating" ? Number(el.value) : el.value; }
      });
    });
    // Кнопки add/del
    $$a("[data-rv-act]", wrap).forEach((btn) => {
      btn.addEventListener("click", () => {
        const si = +btn.dataset.si, act = btn.dataset.rvAct;
        const s = state.reviews.sources;
        if (act === "del-src") { s.splice(si, 1); }
        else if (act === "add-review") { s[si].featured = s[si].featured || []; s[si].featured.push(blankReview()); }
        else if (act === "del-review") { s[si].featured.splice(+btn.dataset.fi, 1); }
        renderReviewsTab();
      });
    });
  }
  ...
  async function saveReviews() {
    const st = $("#rv-status");
    const sources = (state.reviews && state.reviews.sources) || [];
    if (!state.api) {
      if (st) { st.textContent = "Демо-режим: без URL воркера сохранение недоступно. Задайте apiBase / войдите с воркером."; st.className = "text-sm text-apple-red mt-3"; }
      return;
    }
    const btn = $("#rv-save"); const label = btn ? btn.textContent : "";
    if (btn) { btn.disabled = true; btn.textContent = "Сохраняю…"; }
    if (st) { st.textContent = "Сохраняю отзывы в KV…"; st.className = "text-sm text-ink-mute mt-3"; }
    try {
      const r = await api("/api/admin/reviews", { method: "POST", body: JSON.stringify({ sources }) });
      if (st) { st.textContent = `Сохранено: ${r.count} источник(ов). Сайт подхватит при следующей загрузке.`; st.className = "text-sm text-apple-green mt-3"; }
      toast("Отзывы сохранены");
    } catch (e) {
      if (st) { st.textContent = "Не удалось сохранить: " + (e && e.message ? e.message : "ошибка"); st.className = "text-sm text-apple-red mt-3"; }
    } finally {
  ...
  function renderProducts() {
      ...
      .map((g) => {
        ...
        const header = `
          <div class="flex items-center gap-3 px-3 py-2.5 cursor-pointer select-none" data-ghead="${esc(g.key)}">
            ...
            <button class="adm-icbtn shrink-0 ${g.list.every((x) => x.hidden) ? "text-apple-amber" : ""}" data-ghide="${esc(g.key)}" title="${g.list.every((x) => x.hidden) ? "Показать все варианты модели" : "Скрыть модель целиком (останется в каталоге)"}">${svg(g.list.every((x) => x.hidden) ? "eye-off" : "eye")}</button>
        ...
        const rows = !open ? "" : `<div class="border-t border-black/[0.06] divide-y divide-black/[0.04]">${g.list
          .map((p) => `
            <div class="flex items-center gap-2 px-3 py-2 ${p.hidden ? "opacity-50" : ""}" data-id="${esc(p.id)}">
              ...
              <div class="min-w-0 flex-1 text-sm truncate" title="${esc(p.id)}">${esc(variantLabel(p))}${
                p.condition ? ` <span class="chip bg-ink/[0.06] text-ink-soft text-[11px] py-0 px-1.5 align-middle">${esc(condTitle(p.condition))}</span>` : ""
              }</div>
              <label class="hidden sm:flex items-center gap-1 shrink-0 text-[11px] text-ink-mute">цена
                <input type="number" min="0" step="500" value="${Number(p.priceCash) || 0}" data-inline="priceCash" class="adm-input w-28 py-1 text-right"/></label>
              ...
              <button class="adm-icbtn shrink-0 ${p.hidden ? "text-apple-amber" : ""}" data-act="hide" title="${p.hidden ? "Скрыт — показать на сайте" : "Скрыть с сайта"}">${svg(p.hidden ? "eye-off" : "eye", "w-4 h-4")}</button>
    ...
    $$("[data-ghead]", list).forEach((h) => {
      h.onclick = (e) => {
        if (e.target.closest("[data-gadd],[data-grename],[data-gdelete],[data-ghide]")) return;
    ...
    // Скрыть/показать модель ЦЕЛИКОМ. Массовая операция: у Galaxy S24 восемь
    // вариантов, и снимать их с продажи по одному — мучение. Товары остаются
    // в каталоге и в CRM, исчезают только с витрины, из поиска, фидов и
    // пререндер-страниц.
    $$("[data-ghide]", list).forEach((b) => {
      b.onclick = (e) => {
        e.stopPropagation();
        const g = groups.find((x) => x.key === b.dataset.ghide);
        if (!g) return;
        const allHidden = g.list.every((x) => x.hidden);
        pushUndo();
        g.list.forEach((p) => { if (allHidden) delete p.hidden; else p.hidden = true; });
        renderProducts();
        toast(allHidden ? `Показано вариантов: ${g.list.length}` : `Скрыто вариантов: ${g.list.length}`);
      };
    });
    ...
    $$("[data-id]", list).forEach((row) => {
      ...
      $$("[data-inline]", row).forEach((el) => {
        ...
        el.addEventListener("change", () => {
          ...
          pushUndo();
          p[f] = nv;
          if (f === "priceCash") p.priceCard = cardPrice(nv); // карта = наличные +15%
          renderProducts();
          toast("Изменение внесено · Ctrl+Z отменит");
        });
      });
      // Все обработчики строки — через ?. : если одна кнопка почему-то не
      // отрисовалась, падение здесь оборвало бы привязку остальных кнопок
      // строки (именно так перестало работать «Добавить вариант»).
      const hideBtn = row.querySelector('[data-act="hide"]');
      if (hideBtn) hideBtn.onclick = () => {
        // hidden храним только когда true — как и в редакторе варианта, чтобы
        // не засорять каждую позицию полем "hidden": false.
        pushUndo();
        if (p.hidden) delete p.hidden; else p.hidden = true;
        renderProducts();
      };
      row.querySelector('[data-act="cover"]')?.addEventListener("click", () => setCover(coverKey(p), p.color));
      row.querySelector('[data-act="edit"]')?.addEventListener("click", () => openEditor(id));
      row.querySelector('[data-act="dup"]')?.addEventListener("click", () => duplicateProduct(id));
      row.querySelector('[data-act="del"]')?.addEventListener("click", () => deleteProduct(id));
    });
  }
  ...
  function addVariant(key) {
    ...
    if (rep) {
      ...
      d.storage = rep.storage; d.watchSize = rep.watchSize; d.sim = rep.sim; d.region = rep.region || "";
  ...
  function renderEditor() {
    ...
    $("#drawer-body").innerHTML = `
      ...
      ${field("Регион (необязательно)", `<input class="adm-input" data-f="region" list="preset-region" value="${esc(d.region || "")}" placeholder="напр. 🇦🇪 ОАЭ" autocomplete="off"/>
        <datalist id="preset-region">${presetList("region", REGION_BASE, d.region).map((s) => `<option value="${esc(s)}"></option>`).join("")}</datalist>`)}
      ${field("Цена, ₽", `<input type="number" min="0" step="500" class="adm-input" data-f="priceCash" value="${Number(d.priceCash) || 0}"/>`)}
      <div class="grid grid-cols-2 gap-3">
        ${field("Наличие", `<select class="adm-input" data-f="status">${stOpts}</select>`)}
        ${field("Состояние", `<select class="adm-input" data-f="condition">${CONDITION_OPTS
          .map((c) => `<option value="${esc(c.v)}" ${c.v === (d.condition || "") ? "selected" : ""}>${esc(c.t)}</option>`).join("")}</select>`)}
      </div>
      <p class="text-[11px] text-ink-mute -mt-2 mb-3">Б/у и витринные: гарантия 3 месяца проставится сама, в заголовке страницы появится «б/у», в фидах — пометка состояния.</p>
      ${field("Бейдж (необязательно)", `<input class="adm-input" data-f="badge" value="${esc(d.badge || "")}" placeholder="Новинка / Хит"/>`)}

      <label class="mt-3 flex items-start gap-2 rounded-xl bg-cloud p-3 cursor-pointer">
        <input type="checkbox" class="mt-0.5 w-4 h-4 accent-brand" data-f="hidden" ${d.hidden ? "checked" : ""}/>
        <span class="text-sm">
          <span class="font-medium">Скрыть с сайта</span>
          <span class="block text-ink-mute text-xs mt-0.5">
            Товар останется в каталоге и в выгрузках CRM, но исчезнет с витрины,
            из поиска, из карточек и из фидов для ВК, 2ГИС и Яндекс.Карт.
            Цены у него продолжат обновляться. Снять флаг можно в любой момент.
          </span>
        </span>
      </label>
    ...
    $$("[data-f]", $("#drawer-body")).forEach((el) => {
      // change нужен наряду с input: у чекбоксов input в старых движках
      // не всегда срабатывает, а без него флаг «Скрыть с сайта» молча не
      // сохранялся бы.
      ...
      el.addEventListener(evt, () => {
        ...
        if (f === "watchSize") v = String(v).trim() || null;
        // hidden храним только когда он true: незачем засорять каждую из 257
        // позиций полем "hidden": false.
        if (f === "hidden") {
          if (v) state.draft.hidden = true;
          else delete state.draft.hidden;
          return;
        }
        // Состояние: «новое» — это отсутствие поля, а не condition:"new".
        // Так каталог не обрастает лишним полем у 274 новых позиций, и вся
        // витрина проверяет состояние одним способом.
        if (f === "condition") {
          if (v) state.draft.condition = v;
          else delete state.draft.condition;
          return;
        }
        state.draft[f] = v;
        if (f === "priceCash") {
          ...
          if (auto) auto.value = state.draft.priceCard; // карта = наличные +15%
        }
      });
    });
    ...
    list.innerHTML = cats
      .map((c, i) => {
        ...
        const imgCell = c.image
          ? `<img src="${esc(c.image)}" class="w-full h-full object-contain" loading="lazy" onerror="this.style.display='none'"/>`
          : `<span class="text-white grid place-items-center w-full h-full" style="background:${esc(c.accent || "#0582F6")}">${svg("image", "w-4 h-4")}</span>`;
        return `<div class="bg-cloud-card rounded-2xl border border-black/[0.06] shadow-sm p-2" data-key="${esc(c.key)}">
          <div class="flex items-center gap-2">
            <div class="adm-thumb w-10 h-10 shrink-0 overflow-hidden">${imgCell}</div>
            <input class="adm-input font-medium flex-1" data-ct value="${esc(c.title)}"/>
          ...
          <div class="flex items-center gap-2 mt-1.5">
            <span class="text-[11px] text-ink-mute shrink-0 pl-1">Картинка:</span>
            <input class="adm-input flex-1 text-[11px] py-1.5" data-cimg value="${esc(c.image || "")}" placeholder="assets/products/categories/…webp" spellcheck="false"/>
            <label class="btn-pill text-xs cursor-pointer shrink-0">${svg("image", "w-3.5 h-3.5")} Файл<input type="file" accept="image/*" class="hidden" data-cimg-file/></label>
          </div>
        </div>`;
      })
      .join("");

    $$("[data-key]", list).forEach((row) => {
      ...
      row.querySelector("[data-cimg]").addEventListener("change", (e) => { pushUndo(); c.image = e.target.value.trim() || undefined; renderCategories(); toast("Картинка категории обновлена"); });
      row.querySelector("[data-cimg-file]").addEventListener("change", (e) => {
        const f = e.target.files && e.target.files[0]; if (!f) return;
        const rd = new FileReader(); rd.onload = () => { pushUndo(); c.image = rd.result; renderCategories(); toast("Картинка загружена (data-URL). Лучше указать путь к файлу."); }; rd.readAsDataURL(f);
      });
    });
  }
  ...
  function init() {
    ...
    $("#rv-add-src").onclick = () => { state.reviews = state.reviews || { sources: [] }; state.reviews.sources.push(blankSource()); renderReviewsTab(); };
    $("#rv-save").onclick = saveReviews;
