// app.js — Основная логика витрины Apple i Точка
import { CONFIG } from "./config.js";
import { Store } from "./storage.js";
import { SmartSearch } from "./search.js";
import { Compare } from "./compare.js";
import { TradeIn } from "./trade-in.js";
...
async function loadProducts() {
  ...
  try {
    ...
    console.warn("Worker недоступен, использую локальный каталог:", e);
  }
  return local();
}
...
function statusBadge(status) {
  return status === "in_stock"
    ...
    : `<span class="chip chip-amber"><span class="w-1.5 h-1.5 rounded-full bg-apple-amber"></span>Под заказ 1–2 дня</span>`;
}
