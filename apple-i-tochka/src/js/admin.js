...
// classic script: CONFIG и window.__CATALOG__ подключаются раньше в HTML.
...
async function loadProducts() {
  ...
  if (API) {
    ...
    // Демо-режим без Worker — встроенный каталог (сохранение недоступно)
    data = window.__CATALOG__ || { products: [] };
  }
...
async function seedKV() {
  ...
  try {
    ...
    if (!catalog) throw new Error("Встроенный каталог не найден (data/products.js)");
