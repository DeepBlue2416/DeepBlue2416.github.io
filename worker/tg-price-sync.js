...
function matchOffer(offer, products) {
  ...
  // Б/у и витринные позиции из сопоставления исключаем: прайс поставщика —
  // это закупка новых устройств, и подставить её в цену подержанного значило
  // бы переписать цену, выставленную вручную.
  const poolAll = products.filter((p) => String(p.series).toLowerCase() === low && !p.condition);
